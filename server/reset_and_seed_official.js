...
const rawProducts = [
  ...
  {"name": "Pantalla", "brand": "Honor", "model": "X6B", "quality": "Original - Muestra", "qty": 1, "fob": 0.00},
...
// Step 1: Wipe all test data tables cleanly (preserving users)
db.serialize(() => {
  db.run("DELETE FROM batch_items", () => {});
  db.run("DELETE FROM batches", () => {});
  db.run("DELETE FROM inventory", () => {});
  db.run("DELETE FROM price_history", () => {
    console.log("🧹 1. Reset Completo: Tablas de Lotes, Items e Inventario limpiadas.");
    sendSingleBatchRequest();
  });
});

function sendSingleBatchRequest() {
  const batchPayload = {
  ...
  console.log('🚀 2. Inserción Única del Lote Oficial (66 Productos / 510 Unidades)...');
  ...
  const req = http.request(options, (res) => {
    ...
    res.on('end', () => {
      if (res.statusCode >= 200 && res.statusCode < 300) {
        ...
        console.log(`✅ Lote registrado exitosamente con ID: ${parsed.id}`);
        
        // Step 3: Verify Integrity in Inventory
        db.all("SELECT * FROM inventory", [], (err, rows) => {
          ...
          console.log(`• Total Filas en Inventario (Productos Únicos): ${rows.length}`);
          console.log(`• Total Unidades en Stock: ${totalUnits}`);

          // Specific verification for "Pantalla Honor 90 Lite"
          const honorProduct = rows.find(r => r.brand === 'Honor' && r.model === 'Honor 90 Lite');
          if (honorProduct) {
            const landedCost = honorProduct.unitCost;
            const sellingPriceUsd = landedCost * 1.15;
            const sellingPriceGtq = sellingPriceUsd * 7.80;

            console.log(`\n🔍 DETALLE VERIFICADO - Pantalla Honor 90 Lite:`);
            console.log(`  - Existencias en Stock: ${honorProduct.stock} unidades`);
            console.log(`  - Costo FOB Base: $8.90 USD`);
            console.log(`  - Costo Landed Unitario: $${landedCost.toFixed(2)} USD`);
            console.log(`  - Precio Venta (+15%): Q ${sellingPriceGtq.toFixed(2)} GTQ ($${sellingPriceUsd.toFixed(2)} USD)`);
          } else {
            console.error("❌ No se encontró la Pantalla Honor 90 Lite en el inventario.");
          }
        });
      } else {
        console.error(`❌ Error en servidor (${res.statusCode}):`, data);
      }
    });
  });
