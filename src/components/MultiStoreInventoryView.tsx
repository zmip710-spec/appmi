...
export const MultiStoreInventoryView: React.FC<MultiStoreInventoryViewProps> = ({ currentUser }) => {
  ...
  const isSkuDuplicate = useMemo(() => {
    if (!newSku.trim()) return false;
    const clean = newSku.trim().toUpperCase();
    return matrix.some(p => (p.sku || '').trim().toUpperCase() === clean);
  }, [newSku, matrix]);
  ...
  const handleCreateProductSubmit = async (e: React.FormEvent) => {
    ...
    if (isSkuDuplicate) {
      triggerToast(`El código SKU '${newSku.trim().toUpperCase()}' ya existe en el inventario.`, 'error');
      return;
    }
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <div className="flex items-center justify-between mb-1.5">
                        <label className="block text-slate-300 font-semibold uppercase text-xs">
                          Código SKU Único *
                        </label>
            ...
            <div className="fixed inset-0 z-50 w-screen h-screen bg-slate-950 flex flex-col overflow-hidden">
              ...
              <div className="w-full border-b border-slate-800 px-6 py-4 flex items-center justify-between bg-slate-900/50 shrink-0">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-2.5 flex-wrap">
                    <h3 className="text-base sm:text-lg font-bold text-white tracking-tight">Editar Producto</h3>
                    <span className="font-mono text-xs font-bold text-indigo-400 bg-indigo-500/10 px-2.5 py-0.5 rounded-lg border border-indigo-500/20">
                      {selectedDetailProduct.sku}
                    </span>
              ...
              {/* Cuerpo del formulario con scroll limpio */}
              <div className="flex-1 overflow-y-auto px-6 py-6 w-full max-w-5xl mx-auto space-y-6">
                {/* Bloque 1: Información & Precios Comerciales */}
                <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-5 sm:p-6 space-y-5">
                  <div className="flex items-center justify-between pb-3 border-b border-slate-800">
                    ...
                    <div className="inline-flex bg-slate-950 p-0.5 rounded-lg border border-slate-700/60 text-xs">
                      <button
                        type="button"
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full h-10 px-3.5 bg-slate-950 border border-slate-700 rounded-xl text-xs text-white focus:border-indigo-500 focus:outline-none"
                      />
                    </div>
                    <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-1">
                    <div>
                      <label className="block text-xs font-semibold text-slate-300 mb-1.5">Precio Costo (Q)</label>
                      <input
                        type="number"
                        step="0.01"
                        min="0"
                        ...
                        className="w-full h-10 px-3.5 bg-slate-950 border border-slate-700 rounded-xl text-xs text-white font-mono focus:border-indigo-500 focus:outline-none"
                      />
                    </div>
                      ...
                      {editPricingMode === 'fixed' ? (
                        <>
                          <input
                            ...
                            className="w-full h-10 px-3.5 bg-slate-950 border border-slate-700 rounded-xl text-xs text-emerald-400 font-mono focus:border-emerald-500 focus:outline-none"
                          />
                          {(() => {
                            ...
                            return (
                              <div className="mt-1.5 text-xs text-slate-400 font-mono truncate">
                                Margen: <strong className="text-emerald-400">+{pct.toFixed(1)}%</strong> (+Q {profit.toFixed(2)})
                              </div>
                            );
                          })()}
                        </>
                        ...
                        <>
                          <input
                            ...
                            className="w-full h-10 px-3.5 bg-slate-950 border border-emerald-500/40 rounded-xl text-xs text-emerald-400 font-mono focus:border-emerald-500 focus:outline-none"
                          />
                          {(() => {
                            const cost = parseFloat(editCostPrice) || 0;
                            ...
                            const calcSale = cost * (1 + margin / 100);
                            return (
                              <div className="mt-1.5 text-xs text-emerald-400 font-mono font-bold truncate">
                  ...
                  <div className="flex justify-end pt-2 border-t border-slate-800/80">
                    <button
                      ...
                      className="h-9 px-5 text-xs font-semibold rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white inline-flex items-center gap-1.5 transition cursor-pointer disabled:opacity-50 shadow-md shadow-indigo-600/20"
                    >
                      <Check className="w-4 h-4" />
                  ...
                  <form onSubmit={handleEditStockSubmit} className="space-y-4">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            className="flex-1 h-10 px-3.5 bg-slate-950 border border-slate-700 rounded-xl text-xs text-white font-mono focus:border-indigo-500 focus:outline-none"
                          />
                          <button
                            type="button"
                            onClick={() => setEditStockQty(q => q + 1)}
                            className="h-10 px-3 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-xl text-xs font-mono font-semibold shrink-0 cursor-pointer transition border border-slate-700"
                          >
                            +1
                          </button>
                          <button
                    ...
                    <div className="flex justify-end pt-2 border-t border-slate-800/80">
                      <button
                        ...
                        className="h-9 px-5 text-xs font-bold rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white inline-flex items-center gap-1.5 transition cursor-pointer disabled:opacity-50 shadow-md shadow-emerald-600/20"
                      >
                        <Plus className="w-4 h-4" />
                ...
                {currentUser?.role !== 'Vendedor' && (
                  <div className="bg-rose-950/20 border border-rose-500/30 rounded-2xl p-5 sm:p-6 space-y-4">
                    ...
                    <p className="text-xs text-rose-200/80 leading-relaxed">
                      Esta acción es irreversible y eliminará este SKU junto con todas sus existencias e historial registrados en todas las sucursales.
                    </p>
                    ...
                    {!isConfirmingDeleteInEdit ? (
                      <div className="flex justify-end pt-1">
                        <button
                          ...
                          className="h-9 px-4 text-xs font-semibold rounded-xl bg-rose-500/15 text-rose-300 border border-rose-500/40 hover:bg-rose-500/25 inline-flex items-center gap-1.5 transition cursor-pointer"
                        >
                      ...
                      <div className="p-4 bg-rose-950/60 border border-rose-500/50 rounded-xl space-y-3 animate-in fade-in duration-150">
                        <p className="text-xs font-bold text-rose-300">
                          ¿Confirmas que deseas eliminar definitivamente este SKU del sistema?
                        </p>
                        <div className="flex items-center justify-end gap-3">
                          <button
                            type="button"
                            onClick={() => setIsConfirmingDeleteInEdit(false)}
                            className="h-8 px-4 text-xs font-medium text-slate-300 hover:text-white bg-slate-800 rounded-xl transition cursor-pointer"
                          >
                            Cancelar
                          </button>
                          <button
                            ...
                            className="h-8 px-4 text-xs font-bold bg-rose-600 hover:bg-rose-500 text-white rounded-xl transition cursor-pointer disabled:opacity-50 shadow-md shadow-rose-600/30"
                          >
              ...
              <div className="w-full px-6 py-4 border-t border-slate-800 bg-slate-900/90 flex justify-end shrink-0">
                <button
                  type="button"
                  onClick={() => {
                    setShowEditModal(false);
